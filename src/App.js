...
const App = () => {
  ...
  const moveIntoSquareBelow = () => {
    for (let i = 0; i <= 55; i++) {
      const isFirstRow = [0, 1, 2, 3, 4, 5, 6, 7].includes(i);

      if (isFirstRow && currentColorArrangement === blank) {
        let randomNumber = Math.floor(candyColours.length * Math.random());
  ...
  const dragEnd = (e) => {
    ...
    // console.log('ReplacedId:', squareBeingReplacedId)
    // console.log('DraggedId:', squareBeingDraggedId)
  ...
  const createBoard = () => {
    const randomColorArranged = [];
    for (let i = 0; i < width * width; i++) {
      const randomColor =
        candyColours[Math.floor(candyColours.length * Math.random())];
      randomColorArranged.push(randomColor);
    }
    setCurrentColorArrangement(randomColorArranged);
  };
